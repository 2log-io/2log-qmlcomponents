...
Container {
    ...
    property double threshold: controllerModel ? controllerModel.getProperty(
                                                     "activityThreshold").value : 0
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onReleased: {
                        ...
                        controllerModel.getProperty(
                                    "activityThreshold").value = chart.getValueForY(
                                    handle.y)
                    }
                }
                ...
                TextLabel {
                    ...
                    Binding on text {
                        delayed: true
                        value: (chart.maxVal > 0
                                && chart.height > 0) ? chart.getValueForY(
                                                           handle.y).toFixed(
                                                           2) + " A" : 0
                    }
                }
                ...
                SynchronizedListModel {
                    ...
                    onCountChanged: {
                        for (var i = 0; i < count; i++) {
                            ...
                            chart.addData(val >= 0 ? val : 0,
                                          influxmodel.get(i).time,
                                          i === count - 1)
                        }
                    }
                }
    ...
    Flow {
        ...
        Row {
            Form {
                id: form1
                width: 145
                visible: controllerModel && controllerModel.hasProperty(
                             "activityTimeout")
                FormTextItem {
                    id: nameLabel
                    label: qsTr("Zeitliche Toleranz:")
                    Binding on text {
                        value: controllerModel ? controllerModel.getProperty(
                                                     "activityTimeout").value / 1000 : ""
                    }
                    validator: IntValidator {
                        bottom: 0
                        top: 60
                    }
                    onAccepted: if (controllerModel)
                                    controllerModel.getProperty(
                                                "activityTimeout").value = editedText * 1000
                    //  mandatory: true
                }
            }
            TextLabel {
        ...
        Row {
            Form {
                ...
                FormTextItem {
                    ...
                    onAccepted: if (controllerModel)
                                    controllerModel.getProperty(
                                                "machineInitTime").value = editedText * 1000
                }
            }
            TextLabel {
